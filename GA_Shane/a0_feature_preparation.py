...
def gen_feature(df):
    ...
    # log
    df = df.with_columns([
        pl.col(c).log().alias(f'{c}') for c in set1
    ]).with_columns([
    ...
    df = df.with_columns([
        ...
        (pl.col('open')-pl.col('close')).alias('bar_box'),
        (pl.col('high')-pl.col('low')).alias('bar_up'),
        (pl.col('close') - pl.col('mid3')).rolling_std(30).alias('std_ret_mid3'),
    ])
    ...
    df = df.with_columns([
        pl.col(c).shift(1).alias(f'prev_{c}') for c in set1+set2+set3
    ])
