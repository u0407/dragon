...
train_path = glob.glob(os.path.join(dir,f'*_Train_{fix}*.csv'))[0]
test_path = glob.glob(os.path.join(dir,f'*_Test_{fix}*_PCA.csv'))[0]
...
import sys 
sys.getsizeof(dataset_s) / (1024 ** 2)

dataset.head()

def unique_primes(start, n=10):
    def pf(x):
        factors = set()
        while x % 2 == 0:
            factors.add(2)
        ...
        if x > 1:
            factors.add(x)
        return [ f for f in factors if f > 5 ]
    return sorted(set().union(*(pf(x) for x in range(start-n, start+1))))

# Example usage:
param_list = unique_primes(dataset_s.shape[1], 10)
print(param_list)
...
for j in param_list:        ########## 400个特征 #############
    ...
    abc_results.to_csv(os.path.join(dir,'temp',str(j)+'r.csv'),index = False)
    
    final_best = finalize_model(abc)
    save_model(final_best, os.path.join(dir,'temp',str(num)+ 'x'))



    data = pd.read_csv(test_path) #########  测试集文件  ########################
    # data = pd.read_csv('./IC99_1m_MIXED_0.008_11361_4564_Label_513_2023/IC99_1m_MIXED_0.008_11361_4564.csv_tz80_Test_4564_PCA.csv') #########  测试集文件  ########################
...
#     data.replace([np.inf, -np.inf], np.nan, inplace=True)   ####替换正负inf为NA
    ...
    Note=open(os.path.join(dir,'temp',str(num)+ 'x.txt'),mode='a')
    ...
    Note=open(os.path.join(dir,'temp',str(num)+ 's.txt'),mode='a')
    for i in range(0,data_1_size):         
    ...
    file_name = os.path.join(dir,'temp','Show.csv')
    df = pd.read_csv(file_name)
    path = os.path.join(dir,'temp',str(j)+'x.txt')
    df2 = pd.read_csv(path, header=None, names=['state_x'])
    for i in range(0,data_1_size):  
        df['low'][i] = df2['state_x'][i]

    path = os.path.join(dir,'temp',str(j)+'s.txt')
    df2 = pd.read_csv(path, header=None, names=['state_x'])
    df['score'] = 0
    for i in range(0,data_1_size):  
        df['score'][i] = df2['state_x'][i]
    df.to_csv(os.path.join(dir,'temp',str(j)+'x.csv'),index = False)
 

    file_name=os.path.join(dir,'temp',str(j)+'x.csv')
...
######################################################################################################
    ...
    data_1_new.to_csv(os.path.join(dir,'temp',str(j)+'x.csv'),index = False)
    ...
    fig.autofmt_xdate()
    plt.grid(1)
    plt.savefig(os.path.join(dir,'temp',str(j)+'sy.jpg'))
    plt.close()
    ...
    fig.autofmt_xdate()
    plt.grid(1)
    plt.savefig(os.path.join(dir,'temp',str(j)+'p.jpg'))
    plt.close()
...
bbbP.to_csv(os.path.join(dir,'temp','Best_2.csv'),index = False)
...
bbb1.to_csv(os.path.join(dir,'temp','Best_1.csv'),index = False)
